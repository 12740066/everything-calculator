...
function renderHoleInputs() {
  ...
  if (isHoleZero) {
    ...
    if (isLongDriveVisible) {
      html += `<th>Long Drive (m)<br />
               <small style="font-size: 0.8em; color: #666;">Distance to the flag</small>
             </th>`;
    }
      ...
      html += `<tr>
        ...
        <td>
          <div class="number-input">
            ...
            <input id="stroke${i}" type="number" value="${strokes}" />
        ...
        html += `<td>
          <div style="display: flex; align-items: center; gap: 10px;">
            <input id="longDrive${i}" 
                   type="number" 
                   value="${longDrive}" 
                   placeholder="Distance to flag (m)" 
                   ${!fairway ? "disabled" : ""} 
                   oninput="updateLongDrive(${i}, this.value);" />
            <label>
              <input id="fairwayCheckbox${i}" 
                     type="checkbox" 
                     ${fairway ? "checked" : ""} 
                     onchange="toggleFairway(${i}, this.checked);" />
              Fairway
            </label>
          </div>
        </td>`;
      ...
      if (isNtpVisible) {
        html += `<td>
          <div style="display: flex; align-items: center; gap: 10px;">
            ...
            <label>
              <input id="ntpCheckbox${i}" 
                     type="checkbox" 
                     ${ntpChecked ? "checked" : ""} 
                     onchange="toggleNtp(${i}, this.checked);" />
              Green
            </label>
          </div>
        </td>`;
...
function calculateLongDriveEarnings(holeData) {
  ...
  // Special Case: Only women compete
  const onlyWomenCompete = menPar !== 5 && womenPar === 5;

  if (onlyWomenCompete) {
    holeData.longDriveCarryoverStars = 1; // Always reset to 1 for this special case
    calculateLongDriveForWomen(holeData, longDriveDistances, fairwayStatuses, strokes, womenPar);
    return;
  }

  // General Long Drive logic for other scenarios
  calculateLongDriveForGeneralGroup(holeData, longDriveDistances, fairwayStatuses, strokes);
}
...
function calculateLongDriveForGeneralGroup(holeData, longDriveDistances, fairwayStatuses, strokes) {
  ...
  // Filter all eligible players (men and women)
  const eligiblePlayers = players
    .map((player, i) => ({
      index: i,
      name: player.name,
  ...
  if (eligiblePlayers.length === 0) {
    ...
    holeData.longDriveCarryoverStars = holeData.longDriveCarryoverStars + 1 || 1; // Retain carryover level for general cases
    return;
  }

  // Find the shortest distance (winner)
  const shortestDistance = Math.min(...eligiblePlayers.map(player => player.distance));
  const winners = eligiblePlayers.filter(player => player.distance === shortestDistance);

  if (winners.length > 1) {
    console.log("Tie detected for Long Drive. All tied players are winners.");
    const earningsPerWinner = skinValue * (eligiblePlayers.length - winners.length);
  ...
  const winner = winners[0];
  console.log(`Long Drive Winner: ${winner.name}`);
  holeData.starEarnings[winner.index] += skinValue * (eligiblePlayers.length - 1);
